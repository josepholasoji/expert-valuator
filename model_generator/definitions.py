...
#
classifications = """
intermediate,novice,disqualified,expert
"""
...
template_data = """
   {
      "languages": ["java", "javascript", "none"],
      "technology": ["redis", "monogodb", "mssql", "spring", "spring-boot", "apache-camel","nodejs", "angulajs","reactjs","none"],
      "disqualified":[{
         "language": "none",
         "library": "none",
         "imported_library":false,
         "commit_count":[0, 1],
         "days_since_last_commit": [730,731],
         "code_churn": [0,1],
...
def gen_classification_data(classification_template, libraries_dict, language_dict, classifications_dict):
   ...
   libraries = []
   for lib in classification_template['library']:
      lib = lib.lower()
   ...
   for lib_val in libraries:
      entry = []
      for day in range(classification_template['days_since_last_commit'][0], classification_template['days_since_last_commit'][1]):
         for commit in range(classification_template['commit_count'][0], classification_template['commit_count'][1]):
            for code_churn in range(classification_template['code_churn'][0], classification_template['code_churn'][1]):
               if len(libraries) == 1 and libraries[0] == libraries_dict["none"]:
                  entry = [language_val, lib_val, 0, commit, day, code_churn, classifications_dict[classification_template['classification'].lower()]]      
               else:
                  entry = [language_val, lib_val, 1, commit, day, code_churn, classifications_dict[classification_template['classification'].lower()]]                         
